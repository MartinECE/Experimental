...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
         ...
         * @var string
         */
        public $menu = 'omg-install-plugins';
        ...
        public function __construct() {
            ...
            $this->strings = array(
                ...
                'oops'                           => __( 'Something went wrong.', 'omg' ),
         ...
         *
         * We get the links in the admin notices to point to the omg page, rather
         ...
         * @return null Returns early if not the omg page.
         */
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'omg_dismissed_notice', true ) ) {
                add_thickbox();
            }
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new omg_List_Table;

            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'omg-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }
            ...
            <div class="omg wrap">
                ...
                <form id="omg-plugins" action="" method="post">
                    <input type="hidden" name="omg-page" value="<?php echo $this->menu; ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['omg-install'] ) && 'install-plugin' == $_GET['omg-install'] ) ) {
                check_admin_referer( 'omg-install' );
                ...
                $fields = array( 'omg-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'omg' ) . '">' . __( 'Return to the Dashboard', 'omg' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['omg-activate'] ) && 'activate-plugin' == $_GET['omg-activate'] ) ) {
                check_admin_referer( 'omg-activate', 'omg-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'omg' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'omg' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'omg' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'omg_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . add_query_arg( 'omg-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['omg-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'omg_dismissed_notice', 1 );
            }
        ...
        public function actions( $install_actions ) {
            ...
            // Remove action links on the omg install page.
            if ( $this->is_omg_page() ) {
                return false;
            }
        ...
        /**
         * Determine if we're on the omg Install page.
         *
         * @since 2.1.0
         *
         * @return boolean True when on the omg page, false otherwise.
         */
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'omg_dismissed_notice' );
    ...
    $omg = TGM_Plugin_Activation::get_instance();
...
if ( ! class_exists( 'omg_List_Table' ) ) {
    /**
     ...
     */
    class omg_List_Table extends WP_List_Table {

        /**
         * References parent constructor and sets defaults for class.
         *
         * The constructor also grabs a copy of $instance from the omg class
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'omg' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'omg' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'omg' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'omg' ) : __( 'Recommended', 'omg' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'omg' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'omg' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'omg' ) . ' %2$s">' . __( 'Install', 'omg' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'omg' ) . ' %2$s">' . __( 'Activate', 'omg' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
                                'plugin_name'          => $item['sanitized_plugin'],
                                'plugin_source'        => $item['url'],
                                'omg-activate'       => 'activate-plugin',
                                'omg-activate-nonce' => wp_create_nonce( 'omg-activate' ),
                            ),
                            network_admin_url( 'themes.php' )
                        ),
                        $item['sanitized_plugin']
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'omg' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'omg-bulk-install'  => __( 'Install', 'omg' ),
                'omg-bulk-activate' => __( 'Activate', 'omg' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'omg-bulk-install' === $this->current_action() ) {
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'omg-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'omg-empty';
                    $i++;
                }
            ...
            if ( 'omg-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'omg' ), $imploded );
                }
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'omg' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'omg' );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'omg' );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'omg' );
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'omg' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'omg' );
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    ...
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'omg' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'omg' ) . '</span><span class="hidden">' . __( 'Hide Details', 'omg' ) . '</span>.</a>';
                ...
                // Default installation strings.
                    ...
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'omg' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'omg' ) . '</span><span class="hidden">' . __( 'Hide Details', 'omg' ) . '</span>.</a>';
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'omg' ) . '">' . __( 'Return to the Dashboard', 'omg' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
